Когда мы рассматривали архитектуру Flutter-приложения, — основные слои и их компоненты, — то вскользь упомянули про Foundation. Продублируем изображение, чтобы вы были в контексте.

![flutter_3_4.1](https://yastatic.net/s3/education-portal/media/fluttern_1_8_2_fb6d0e543f_b9a9ed3491.svg)

Мы выяснили, что  Foundation — это низкоуровневые классы-утилиты, которые используются другими слоями фреймворка, а ещё тут происходит самое важное — связывание приложения со слоем Engine.

За связывание как раз отвечают `сервисы связи` (англ. Bindings). В этом параграфе мы разберём, что это такое, какие сервисы связи бывают, за что они отвечают и как связаны с рендерингом.

## Определение сервисов связи

Начнём чуть издалека. Вы помните, что запуск каждого Flutter-приложения начинается с вызова функции [`runApp`](https://api.flutter.dev/flutter/widgets/runApp.html).
...
`Сервисы связи` (англ. Bindings) — это некоторые связующие классы, выполняющие роль «клея» между движком и фреймворком. А если говорить формальным языком — интерфейсы обмена данными между Flutter framework и Flutter engine.

Вот уже знакомая вам схема — для наглядности и контекста.

![flutter_3_4.2](https://yastatic.net/s3/education-portal/media/fluttern_1_8_1_1a036f3b57_c2c8033e88.svg)
...
## Какие бывают сервисы связи

Базовый класс для всех сервисов связи — [`BindingBase`](https://api.flutter.dev/flutter/foundation/BindingBase-class.html). Конкретные сервисы наследуются именно от него.

Они обязуются гарантировать единственность своего экземпляра и инициализируются только один раз (реализуют паттерн «[Синглтон](https://ru.wikipedia.org/wiki/%D0%9E%D0%B4%D0%B8%D0%BD%D0%BE%D1%87%D0%BA%D0%B0_(%D1%88%D0%B0%D0%B1%D0%BB%D0%BE%D0%BD_%D0%BF%D1%80%D0%BE%D0%B5%D0%BA%D1%82%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F))»). Каждый сервис отделяет в себе обработку ограниченного набора задач, связанных непосредственно с ним. В сервисе `GestureBinding`, например, обрабатываются задачи, связанные со взаимодействием пользователя с экраном устройства.
...
![flutter_3_4.3](https://yastatic.net/s3/education-portal/media/flt_2_2_1e49c1e906_3c8f5aae9a.svg)
...
Для этого мы можем воспользоваться двумя способами — методом `addPostFrameCallback` или геттером `endOfFrame`.
...
{% cut "Пример использования `TimingsCallback`:" %}
...
И ещё один совет: всегда замеряйте метрики производительности до и после изменений. Подробнее о производительности приложения мы расскажем [в одном из дальнейших параграфов](https://education.yandex.ru/handbook/flutter/article/renderobject-pervoe-pogruzhenie).
...
- Создание [`RestorationManager`](https://api.flutter.dev/flutter/services/RestorationManager-class.html) — это сущность, которая отвечает за восстановление состояния приложения. Подробнее про него можно узнать в [лекции](https://www.youtube.com/watch?v=xManvu0z4mU&list=PLIh9yLdjK2YepDyvvvHR-_Iv_YD-W-gSP&index=4) Школы мобильной разработки Яндекса.
...
Получаемые на вход данные о нажатиях доставляются конкретным потребителям этих событий (кнопки, области со скроллом и та далее). Процесс распознавания адресата для события называется `hitTest`, результат распознавания — [`hitTestResult`](https://api.flutter.dev/flutter/gestures/HitTestResult-class.html).
...
Подробнее про `hitTest` мы поговорим в одном [из следующих параграфов](https://education.yandex.ru/handbook/flutter/article/renderobject-pervoe-pogruzhenie).
...
- Передача во Flutter engine изменений на экране с помощью Layer tree. Подробнее об этом также расскажем в [одном из следующих параграфов](https://education.yandex.ru/handbook/flutter/article/renderobject-pervoe-pogruzhenie).

Чтобы передавать изменения в `engine`, этот сервис связи отвечает за управление [`PipelineOwner`](https://api.flutter.dev/flutter/rendering/PipelineOwner-class.html) и инициализацию [`RenderView`](https://api.flutter.dev/flutter/rendering/RenderView-class.html).
...
Связывает `engine` и слой семантики. Отвечает за всё необходимое для `доступности` (англ. ccessibility) приложения, чтобы им могли пользоваться люди с ограниченными возможностями здоровья:
...
Подробнее про `доступность` мы также расскажем в [одном из следующих параграфов](https://education.yandex.ru/handbook/flutter/article/dostupnost).

### PaintingBinding

Сервис для связи с библиотекой `painting`, вот за что он отвечает:
...
## Рендеринг и сервисы связи

Давайте вспомним с вами несколько фактов об устройстве вёрстки во Flutter, которые мы [узнали](https://education.yandex.ru/handbook/flutter/article/elements) ранее в этой главе, когда разбирали элементы:
...
1. Изменения геометрии `RenderObject` (позиция, размер и так далее).
2. Необходимость перерисовки (если поменялся только цвет, стиль шрифта и так далее).
...
![flutter_3_4.4](https://yastatic.net/s3/education-portal/media/flt_2_2_1_14_fcf62d91b2_ad4d72b0f4.svg)

Scheduler делегирует вызов в сервис `WidgetsBinding`, вызывая метод `drawFrame`.

В первую очередь `WidgetsBinding` рассматривает изменения, произошедшие в дереве элементов: вызывает у`BuildOwner` метод `buildScope`, в котором проходится список элементов, помеченных как dirty.

У каждого элемента вызывается метод `rebuild`, что, как правило, ведёт к вызову метода `build` и получению нового виджета. Далее есть два варианта поведения:
...
После обработки элементов подходит очередь рендер-объектов (RenderObject), которые требуют перерисовки. Сервис `WidgetsBinding` по цепочке вызывает метод `drawFrame` у `RendererBinding`, и происходит следующее:
...
![flutter_3_4.5](https://yastatic.net/s3/education-portal/media/flt_2_2_2_13_301c68bfd3_c50349e290.svg)

Совместив весь процесс отрисовки кадра, получаем следующую схему:

![flutter_3_4.6](https://yastatic.net/s3/education-portal/media/flt_2_2_3_e5e7be16a5_19f54aca99.svg)

---

Знаем, было нелегко, но мы справились!

В этом параграфе мы узнали, что такое сервисы связи и как они «склеивают» `Flutter engine` с `Flutter framework`. Изучили, какие бывают типы сервисов связи и какую функцию выполняет каждый из них, а также познакомились с тем, какую роль они принимают в процессе рендеринга.

А в следующем параграфе мы начнём разговор об анимациях во Flutter.
