Dart — объектно-ориентированный язык, так что эта парадигма здесь реализована схоже с другими языками. 

В этом параграфе мы обсудим особенности ООП в Dart, которые на первый взгляд могут показаться непонятными: 
...
Если как следует разобраться в них, они помогут создавать лаконичный и структурированный код. 
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/1/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/2/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/3/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/4/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/5/main.dart" height="500")
...
Поскольку конструктор тоже метод, для него правило такое же. Чтобы его соблюсти, в языке есть конструкторы `factory`. Этот инструмент особенно полезен в двух случаях: 

1. Если нужно определить специфический интерфейс для создания экземпляров.
2. Если нужно реализовать Union-тип.

Покажем на примерах. 

### Пример №1: определяем специфический интерфейс для создания экземпляров
...
Также для таких конструкторов можно использовать ключевое слово `const`.
...
В отличие от большинства языков, в Dart нет ключевого слова `interface`. Если быть точным, то изначально оно было, но в 2012 году разработчики его [удалили](https://news.dartlang.org/2012/06/proposal-to-eliminate-interface.html), и взамен у каждого класса в Dart появился `неявный интерфейс` (англ. [implicit interface](https://dart.dev/guides/language/language-tour#implicit-interfaces)).

> 👉  То есть — в Dart есть только классы и их разновидности: `mixin` и `abstract class`. При дальнейшем объяснении мы будем ссылаться на это знание.

Давайте попробуем доработать пример с `Printer`:

/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/6/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/7/main.dart" height="500")
...
Прежде чем мы поговорим про расширение классов, важно остановиться на наследовании в Dart. В документации сказано, что в языке реализовано наследование на основе `mixin` (англ. [mixin-based inheritance](https://dart.dev/language/classes), подробнее о нём поговорим в этом параграфе, но чуть позже).

Это значит, что каждый класс может иметь только один суперкласс. Это существенное ограничение, но язык предоставляет нам взамен способы расширять поведение класса, о которых пойдёт речь ниже.
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/8/main.dart" height="500")
...
Методы расширения — очень полезный инструмент языка. Они позволяют расширять поведение классов без создания подклассов и Utils-классов. Методы расширения имеют полный доступ к публичному интерфейсу класса, на который пишутся. 
...
Таким образом, нам не придётся писать классы Utils со статическими методами для расширения поведения класса `String`. А полные возможности `extension` можно увидеть в [документации](https://dart.dev/language/extension-methods).

### Ключевое слово with и класс Mixin
...
Команда Dart старается по максимуму защитить разработчика от совершения ошибок — такая схема спасает от проблем множественного наследования. Например, от [ромбовидного наследования](https://ru.wikipedia.org/wiki/Ромбовидное_наследование).
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/9/main.dart" height="500")
...
/iframe/(src="https://education-dartpad-prod.s3-website.yandex.net/?path=1.2.2_dart_oop/10/main.dart" height="500")
...
Отлично, с ООП разобрались. Чтобы закрепить знания, советуем пройти квиз ниже. 

И скорее переходите к следующему параграфу — в нём мы подробно разберём асинхронность в Dart.
